...
- 恢复结构化报表类型、日期参数、Excel Blob 导出、ECharts 6 按需图表和当前 Cupertino 页面风格。
...
## 2026-08-20 全端遗留清理与安全升级

- 删除失效的工作台统计接口封装、对应旧类型、禁用导航样式和未使用的本地会话键。
- 登录失效后保留原访问地址，重新登录后安全返回原页面；拒绝协议相对形式的异常跳转目标。
- Axios 升级到 1.19.0、Element Plus 升级到 2.14.4、ECharts 升级到 6.1.0、vue-echarts 升级到 8.1.0、Vite 升级到 7.3.6。
- npm 官方安全审计为 0 个漏洞；`npm run build` 通过，保留现有大包体积提醒。
