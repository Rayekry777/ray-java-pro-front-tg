...
- Vue 路由页面已接入 `route-stage` 过渡，标题、指标/筛选和主工作区按实验室 `motion-fast/base/slow` 令牌错峰入场。
- 指标数字与购物车数量使用一次性弹性反馈；任务悬停使用一次性扫光；商品卡、后厨票据和通用按钮使用轻量悬停/按压反馈。
- 侧栏聚焦态不再使用整块 Brass 背景，改为深色半透明聚焦面、左侧短 Brass 导轨、轻微阴影和图标高亮；窄屏底部导航将导轨移到底部。
- `prefers-reduced-motion: reduce` 会将非必要动画和过渡缩短至近乎即时，并关闭路由入场动画。
- 状态证据：`tool/qa-motion-sidebar.png`、`tool/qa-motion-sidebar-1024.png`。
