...
function isSelected(row:any){return selected.value.some(item=>item.id===row.id)}
function toggleSelected(row:any,checked:boolean){
  selected.value=checked?[...selected.value.filter(item=>item.id!==row.id),row]:selected.value.filter(item=>item.id!==row.id)
}
...
<template><div class="page"><header class="page-head"><div><p class="eyebrow">{{kind==='employees'?'EMPLOYEE ACCESS':'PRODUCT CATALOG'}}</p><h1>{{kind==='employees'?config.title:'商品管理'}}</h1><p>{{kind==='employees'?'管理员工资料，并从员工权限入口继续配置岗位与门店。':'分类、菜品和套餐统一使用商品权限与真实分页接口。'}}</p></div><div v-if="canManage" class="title-actions"><el-button v-if="['dishes','setmeals'].includes(kind)" type="danger" plain :disabled="!selected.length" @click="removeRows(selected)">批量删除</el-button><el-button type="primary" @click="createCurrent">新增{{config.title.replace('管理','')}}</el-button></div></header>
  ...
  <section v-if="['dishes','setmeals'].includes(kind)" v-loading="loading" class="product-grid">
    <article v-for="row in rows" :key="row.id" class="product-card panel">
      <header><el-tag :type="row.status===1?'success':'warning'">{{row.status===1?'起售':'停售'}}</el-tag><el-checkbox v-if="canManage" :model-value="isSelected(row)" aria-label="选择商品" @change="toggleSelected(row,Boolean($event))"/></header>
      <h3>{{row.name}}</h3><p class="muted">{{row.categoryName || (kind==='dishes'?'未分类菜品':'门店套餐')}}</p>
      <footer><strong>¥{{Number(row.price||0).toFixed(2)}}</strong><div><el-button v-if="canManage" @click="editCurrent(row)">编辑</el-button><el-button v-if="canManage" :type="row.status===1?'danger':'primary'" plain :loading="changing===row.id" @click="toggle(row)">{{row.status===1?'停售':'起售'}}</el-button></div></footer>
    </article>
    <el-empty v-if="!loading&&!rows.length" description="暂无商品"/>
  </section>
  <section v-else-if="kind==='categories'" v-loading="loading" class="panel endpoint-list resource-category-list">
    <article v-for="row in rows" :key="row.id" class="endpoint-row"><strong>{{row.name}}</strong><span class="muted">商品分类 · 排序 {{row.sort}}</span><div><el-tag :type="row.status===1?'success':'info'">{{row.status===1?'启用':'禁用'}}</el-tag><el-button v-if="canManage" @click="editCurrent(row)">编辑</el-button><el-button v-if="canManage" :loading="changing===row.id" @click="toggle(row)">{{row.status===1?'禁用':'启用'}}</el-button></div></article>
    <el-empty v-if="!loading&&!rows.length" description="暂无分类"/>
  </section>
  <section v-else class="panel table-panel"><el-table :data="rows" v-loading="loading" empty-text="暂无数据" @selection-change="selected=$event">
    <el-table-column prop="name" label="名称" min-width="150"/><el-table-column prop="username" label="用户名" min-width="130"/><el-table-column prop="phone" label="手机号" min-width="130"/><el-table-column prop="updateTime" label="更新时间" min-width="170"/>
    <el-table-column label="状态" width="110"><template #default="{row}"><el-tag :type="row.status===1?'success':'info'">{{row.status===1?'启用':'禁用'}}</el-tag></template></el-table-column>
    <el-table-column label="操作" :width="canManage?240:100"><template #default="{row}"><template v-if="canManage"><el-button link type="primary" @click="editCurrent(row)">编辑</el-button><el-button link @click="password(row)">改密</el-button><el-button link :type="row.status===1?'danger':'primary'" :loading="changing===row.id" @click="toggle(row)">{{row.status===1?'禁用':'启用'}}</el-button></template><span v-else class="muted">只读</span></template></el-table-column>
  </el-table></section>
  <el-pagination class="resource-pagination" v-model:current-page="query.page" v-model:page-size="query.pageSize" layout="total, prev, pager, next" :total="total" @current-change="load"/>
