...
const business=ref<BusinessData>(), dineIn=ref<DineInOverview>(), delivery=ref<OrderStatistics>(), shopStatus=ref(0);
const metrics=computed(()=>[
  ["今日总营业额",`¥${(business.value?.turnover||0).toLocaleString("zh-CN",{minimumFractionDigits:2})}`,"全店"],
  ["今日有效订单",String(business.value?.validOrderCount||0),"堂食 + 外卖"],
  ["平均客单价",`¥${(business.value?.unitPrice||0).toFixed(2)}`,"全渠道"],
  ["在店桌台",String(dineIn.value?.occupiedTables||0),"桌"],
  ["配送中",String(delivery.value?.deliveryInProgress||0),"单"]
]);
async function load(){
  ...
  if(results[2].status==="fulfilled")delivery.value=results[2].value;else errors.value.push(`外卖数据：${errorMessage(results[2].reason)}`);
...
<template><div class="page dashboard-overview" v-loading="loading">
  ...
  <section class="channel-grid">
    ...
    <article class="channel-card delivery-channel"><header><div><p class="eyebrow">DELIVERY</p><h2>外卖运营</h2></div><router-link to="/delivery/orders">进入订单 →</router-link></header><div class="channel-stats"><div><span>待接单</span><strong>{{delivery?.toBeConfirmed||0}}</strong></div><div><span>待配送</span><strong>{{delivery?.confirmed||0}}</strong></div><div><span>配送中</span><strong>{{delivery?.deliveryInProgress||0}}</strong></div><div><span>今日有效订单</span><strong>{{business?.validOrderCount||0}}</strong></div></div><footer><router-link class="button primary" to="/delivery/orders">处理外卖订单</router-link><router-link class="button secondary" to="/reports">查看经营报表</router-link></footer></article>
  </section>
  <section class="panel attention-strip"><div><p class="eyebrow">NOW</p><h2>当前需要关注</h2></div><ul><li><strong>{{dineIn?.waitingKitchenItems||0}}</strong><span>道菜等待制作</span></li><li><strong>{{dineIn?.readyToServeItems||0}}</strong><span>道菜等待上桌</span></li><li><strong>{{delivery?.toBeConfirmed||0}}</strong><span>笔外卖等待接单</span></li></ul></section>
</div></template>
